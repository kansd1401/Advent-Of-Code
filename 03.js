...
const wiring = (l1,l2) => {
  ...
  for(path of l1){
    ...
    let instruction = path.split('')
    let direction = instruction.splice(0,1)[0];
  ...
  for(let i = 0;i<path1.length;i++){
    console.log(`lenght of path1:${i} of ${path1.length}`)
    for(let j = 0;j<path2.length;j++){
      if(path1[i][0] === path2[j][0] && path1[i][1] === path2[j][1]){
        common.push([...path1[i],i + j + 2])
      }
    }
  }
  for(point of common){
    if(distance){
      if(distance >  Math.abs(point[0]) + Math.abs(point[1])){
        distance =  Math.abs(point[0]) + Math.abs(point[1])
      }
      if(steps > point[2]){
        steps = point[2]
      }
